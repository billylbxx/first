...
void EXTI0_IRQHandler(void)          //KEY1  PA0
{
	...
	switch(key_flag)
	{
		case 1: TIM_ITConfig(TIM6,TIM_IT_Update,DISABLE);break;//让定时中断失能，即暂停定时器
		case 2: time=hour;
						hour_flag=1;
						printf("hour=%d\r\n",time);break;
		case 3: hour=time;
		...
		case 0:	sec=time;
						TIM_ITConfig(TIM6,TIM_IT_Update,ENABLE);break;//使能定时中断,即开启定时器
	}
}
...
void EXTI15_10_IRQHandler(void)      //KEY2    PC13
{
	...
	if(EXTI_GetITStatus(EXTI_Line13) ==1)
	{
		...
		if(EXTI_GetITStatus(EXTI_Line13) ==1)
		{
			EXTI_ClearITPendingBit(EXTI_Line13);//清标志位
			LED2=~LED2;
			time++;
			if(hour_flag==0)
			{
...
void EXTI1_IRQHandler(void)       //KEY3    PB1
{
	if(EXTI_GetITStatus(EXTI_Line1) ==1)
	{
		...
		if(EXTI_GetITStatus(EXTI_Line1) ==1)
		{
			EXTI_ClearITPendingBit(EXTI_Line1);//清标志位
			LED3=~LED3;
			time--;
			if(hour_flag==0)
			{
