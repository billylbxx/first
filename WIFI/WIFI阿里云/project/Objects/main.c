	...
	while(1){
		...
		if(Usart2.RecFlag){//风扇，LED
			...
			{			
					LED2= 0;
					printf("已为你打开灯\r\n");
					sprintf(shuju4,"%s","已为你打开灯");	
			}		
			else if(strstr((char*)Usart2.RxBuff, "LED2OFF"))
			{			
					LED2= 1;
					printf("已为你关闭灯\r\n");
					sprintf(shuju4,"%s","已为你关闭灯");
			}
			...
			else if(strstr((char*)Usart2.RxBuff, "\"Speed\":2"))
			{
				MOTOR_PWM(300);
				printf("风扇2档\r\n");
				sprintf(shuju5,"%s","风扇2档");
			}
			else if(strstr((char*)Usart2.RxBuff, "\"Speed\":3"))
			{
				MOTOR_PWM(500);
				printf("风扇3档\r\n");
				sprintf(shuju5,"%s","风扇3档");
			}
			else qza(sum);
			
		
		
		LCD_FLASH_SHOW_CHINESE_STR(0,64,shuju5,LCD_BROWN,LCD_WHITE,32);
		LCD_FLASH_SHOW_CHINESE_STR(0,96,shuju4,LCD_LGRAYBLUE,LCD_WHITE,32);
