...
{
		int i ;
	for( i=0; i<=7 ;i++) buff_RGB[LEDid][i]  = ((green  & 0xff) & (1<< (7-i)))  ? (RGB_1):(RGB_0);
	for( i=8; i<=15 ;i++) buff_RGB[LEDid][i]  = ((red & 0xff) & (1<< (15-i)))  ? (RGB_1):(RGB_0);
	for( i=16; i<=23 ;i++) buff_RGB[LEDid][i]  = ((blue & 0xff) & (1<< (23-i)))  ? (RGB_1):(RGB_0);
}
