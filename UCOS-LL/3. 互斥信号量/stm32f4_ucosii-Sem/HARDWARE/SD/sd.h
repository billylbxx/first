...
#define SD_CS(x) if(x)GPIOF->ODR|=1<<6; else  GPIOF->ODR&=~(1<<6);  //片选信号
...
u8 SD_Init(void);							            				//初始化
u8 SD_Read_Data(u8*buf,u32 sector,u32 cnt);		    //读块--读扇区
u8 SD_Write_Data(u8*buf,u32 sector,u32 cnt);		  //写块--写扇区
u32 SD_Get_Sector_Count(void);   					        //读扇区数--获取总扇区
