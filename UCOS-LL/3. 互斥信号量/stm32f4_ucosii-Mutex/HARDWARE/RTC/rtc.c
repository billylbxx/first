#include "rtc.h"
#include "delay.h"
#include "usart.h"



RTC_TIME time_date;
u16 time_set_flag=0x5555; //是否第一次设置时间标志

/**************************
//等待RTC同步
//返回值:0,成功;1,失败;
****************************/
u8 waitint_rtc_synchro(void)
{ 
	u32 timeout=0XFFFFF; 
	//关闭RTC寄存器写保护
	RTC->WPR=0xCA;
	RTC->WPR=0x53; 
	RTC->ISR&=~(1<<5);		//清除RSF位 
	while(timeout&&((RTC->ISR&(1<<5))==0x00))//等待影子寄存器同步
	{
		timeout--;
	}
    if(timeout==0)return 1;	//同步失败 
	RTC->WPR=0xFF;			//使能RTC寄存器写保护  
	return 0;
}

/****************************
RTC初始化模式
返回值:0,成功;1,失败;
******************************/
u8 rtc_init_mode(void)
{ 
	u32 timeout=0X10000; 
	RTC->ISR|=1<<7;	//进入RTC初始化模式
	while(timeout&&((RTC->ISR&(1<<6))==0x00))//等待进入RTC初始化模式成功
	{
		timeout--;
	}
    if(timeout==0)return 1;	//同步失败
	else return 0; 			//同步成功 
}


/*****************************
//十进制转换为BCD码
//val:要转换的十进制数
//返回值:BCD码
*****************************/
u8 dec_to_bcd(u8 val)
{
	u8 bcdhigh = 0; 
	while(val>=10)
	{
		bcdhigh++;
		val-=10;
	} 
	return ((u8)(bcdhigh<<4)|val);
}
/******************************
//BCD码转换为十进制数据
//val:要转换的BCD码
//返回值:十进制数据
**********************/
u8 bcd_to_dec(u8 val)
{
	u8 temp=0;
	temp=(val>>4)*10;
	return (temp+(val&0X0F));
}

/************************
//设置时间
//year,month,date:年(0~99),月(1~12),日(0~31)
//week:星期(1~7,0,非法!)
//hour,min,sec:小时,分钟,秒钟
//ampm:AM/PM,0=AM/24H,1=PM.
//返回值:0,成功
//       1,进入初始化模式失败 
***************************/
u8 set_rtc_time(u8 year,u8 month,u8 date,u8 week,u8 hour,u8 min,u8 sec,u8 ampm)
{
	u32 temp=0;
	//关闭RTC寄存器写保护
	RTC->WPR=0xCA;
	RTC->WPR=0x53; 
	if(rtc_init_mode())return 1;//进入RTC初始化模式失败
	temp=(((u32)week&0X07)<<13)|((u32)dec_to_bcd(year)<<16)|((u32)dec_to_bcd(month)<<8)|(dec_to_bcd(date)); 
	RTC->DR=temp;				//把年月入写入DR寄存器
	temp=0;
	temp=(((u32)ampm&0X01)<<22)|((u32)dec_to_bcd(hour)<<16)|((u32)dec_to_bcd(min)<<8)|(dec_to_bcd(sec));
	RTC->TR=temp; 					//把时分秒写入TR寄存器
	RTC->ISR&=~(1<<7);			//退出RTC初始化模式 
	return 0; 
}

/***********************************
//获取时间
year,mon,date:年,月,日
week:星期
hour,min,sec:小时,分钟,秒钟 
ampm:AM/PM,0=AM/24H,1=PM.
***************************************/
void get_rtc_time(RTC_TIME *p)
{
	u32 temp=0;
	while(waitint_rtc_synchro());	//等待同步  	 
	temp=RTC->DR;
	p->year	=bcd_to_dec((temp>>16)&0XFF);
	p->month=bcd_to_dec((temp>>8)&0X1F);
	p->date	=bcd_to_dec(temp&0X3F);
	p->week	=(temp>>13)&0X07; 
	temp=0;
 	while(waitint_rtc_synchro());	//等待同步  	 
	temp=RTC->TR;
	p->hour=bcd_to_dec((temp>>16)&0X3F);
	p->min=bcd_to_dec((temp>>8)&0X7F);
	p->sec=bcd_to_dec(temp&0X7F);
	p->ampm=temp>>22; 
}

/**************************************
RTC初始化
返回值:0,初始化成功;
       1,LSE开启失败;
       2,进入初始化模式失败;
****************************************/
u8 rtc_init(void)
{
	u16 timeout=0XFFFF; 			//设置超时
	RCC->APB1ENR|=1<<28;			//使能电源接口时钟
	PWR->CR|=1<<8;						//后备区域访问使能(RTC+SRAM)
	if(RTC->BKP0R!=time_set_flag)		//是否第一次配置
	{
		RCC->BDCR|=1<<0;											//开启 LSE 时钟
		while(timeout&&((RCC->BDCR&0X02)==0)) //等待LSE就绪
		{
			timeout--;
			delay_ms(5);
		}	
		if(timeout==0)return 1;		//LSE 开启失败. 
		
		RCC->BDCR &=~(3<<8);	//配置时钟源
		RCC->BDCR|=1<<8;			//选择LSE作为RTC的时钟
		RCC->BDCR|=1<<15;			//使能RTC时钟
		
 		//关闭RTC寄存器写保护
		RTC->WPR=0xCA;
		RTC->WPR=0x53; 
		if(rtc_init_mode())return 2;//进入RTC初始化模式
		RTC->PRER=0XFF;					//同步分频系数(0~7FFF),必须先设置同步分频,再设置异步分频,Frtc=Fclks/((Sprec+1)*(Asprec+1))
		RTC->PRER|=0X7F<<16;		//异步分频系数(1~0X7F)
		RTC->CR&=~(1<<6);				//设置为,24小时格式
		RTC->ISR&=~(1<<7);			//退出初始化模式
		RTC->WPR=0xFF;					//使能寄存器写保护 
		
		set_rtc_time(16,12,19,1,22,22,20,0);		//设置时间
		RTC->BKP0R=time_set_flag;		//标记已经初始化过了
	} 
	return 0;
}

/**********************************************************
周期性唤醒定时器设置
wksel:000,RTC/16;001,RTC/8;010,RTC/4;011,RTC/2;
      10x,ck_spre,1Hz;11x,1Hz,且cnt值增加2^16(即cnt+2^16)
注意:RTC就是RTC的时钟频率,即RTCCLK!
cnt:自动重装载值.减到0,产生中断.
***********************************************************/
void set_rtc_wakeup_time(u8 wksel,u16 cnt)
{ 
	//关闭RTC寄存器写保护
	RTC->WPR=0xCA; 
	RTC->WPR=0x53;
	RTC->CR&=~(1<<10);			//关闭WAKE UP
	while((RTC->ISR&0X04)==0);	//等待WAKE UP修改允许
	RTC->CR&=~(7<<0);			//清除原来的设置
	RTC->CR|=wksel&0X07;		//设置新的值
	RTC->WUTR=cnt;				//设置WAKE UP自动重装载寄存器值
	RTC->ISR&=~(1<<10);			//清除RTC WAKE UP的标志
	
	RTC->CR|=1<<14;				//开启WAKE UP 定时器中断　
	
	EXTI->PR=1<<22;  			//清除LINE22上的中断标志位  
	EXTI->IMR|=1<<22;			//开启line22上的中断 
	EXTI->RTSR|=1<<22;			//line22上事件上升降沿触发 
	NVIC_SetPriority(RTC_WKUP_IRQn,NVIC_EncodePriority(7-2,2,3));  //设置优先级
	NVIC_EnableIRQ(RTC_WKUP_IRQn); //外部中断使能（系统中断没有这个使能）
	RTC->CR|=1<<10;				//开启WAKE UP 定时器
	RTC->WPR=0XFF;				//禁止修改RTC寄存器 
}

#include "ucos_ii.h"
//RTC WAKE UP中断服务函数
void RTC_WKUP_IRQHandler(void)
{ 
	OSIntEnter();					    	//进入中断
	EXTI->PR|=1<<22;	//清除中断线22的中断标志   
	if(RTC->ISR&(1<<10))//WK_UP中断
	{ 
		RTC->ISR&=~(1<<10);	//清除中断标志
		get_rtc_time(&time_date);
		printf("%d:%d:%d\r\n",time_date.hour,time_date.min,time_date.sec);
	}   
	OSIntExit();       	 				//触发任务切换软中断	
}

/************************************************
设置闹钟时间(按星期闹铃,24小时制)
week:星期几(1~7)
hour,min,sec:小时,分钟,秒钟
**************************************************/
void set_rtc_alarm_time(u8 week,u8 hour,u8 min,u8 sec)
{ 
	//关闭RTC寄存器写保护
	RTC->WPR=0xCA; 
	RTC->WPR=0x53;
	RTC->CR&=~(1<<8);	//关闭闹钟A
	while((RTC->ISR&0X01)==0);//等待闹钟A修改允许 
	RTC->ALRMAR=0;		//清空原来设置
	RTC->ALRMAR|=1<<30;	//按星期闹铃 
	RTC->ALRMAR|=0<<22;	//24小时制
	RTC->ALRMAR|=(u32)dec_to_bcd(week)<<24;//星期设置
	RTC->ALRMAR|=(u32)dec_to_bcd(hour)<<16;//小时设置
	RTC->ALRMAR|=(u32)dec_to_bcd(min)<<8;	//分钟设置
	RTC->ALRMAR|=(u32)dec_to_bcd(sec);		//秒钟设置  
	RTC->ALRMASSR=0;	//不使用SUB SEC
	RTC->CR|=1<<12;		//开启闹钟A中断
	RTC->CR|=1<<8;		//开启闹钟A 
	RTC->WPR=0XFF;		//禁止修改RTC寄存器 
	
	RTC->ISR&=~(1<<8);	//清除RTC闹钟A的标志
	EXTI->PR=1<<17;  	  //清除LINE17上的中断标志位  
	EXTI->IMR|=1<<17;	  //开启line17上的中断 
	EXTI->RTSR|=1<<17;	//line17上事件上升降沿触发 

	NVIC_SetPriority(RTC_Alarm_IRQn,NVIC_EncodePriority(7-2,2,2));  //设置优先级
	NVIC_EnableIRQ(RTC_Alarm_IRQn); //外部中断使能（系统中断没有这个使能）
}
//RTC闹钟中断服务函数
void RTC_Alarm_IRQHandler(void)
{    
	if(RTC->ISR&(1<<8))//ALARM A中断
	{
		RTC->ISR&=~(1<<8);	//清除中断标志
	}   
	EXTI->PR|=1<<17;	//清除中断线17的中断标志 											 
}
