...
void start_task(void *pdata)
{
	...
	//创建消息队列
    CommQ=OSQCreate (&CommMsg[0], sizeof(CommMsg)/sizeof(void*));//返回一个指向消息队列事件控制块的指针
...
void OS_TaskLED (void *p_arg)
{
    ...
    while(1)
    {
		...
		{
			printf("Rx Msg:\r\n%s\r\n",mQ_msg);
            if(strcmp(mQ_msg,"Toggle LED1")==0) //比较获取到的消息队列内容
			{
				GPIO_ToggleBits(GPIOC, GPIO_Pin_4);     //LED1
			}
			else if(strcmp(mQ_msg,"Toggle LED2")==0) //比较获取到的消息队列内容
			{
				GPIO_ToggleBits(GPIOC, GPIO_Pin_5);
			} 
			else if(strcmp(mQ_msg,"Toggle LED3")==0) //比较获取到的消息队列内容
			{
    ...
    while(1)
    {
        ...
        case (1<<0)|0x80:
            if(!key1)   /*等待按键释放*/
            {
                ...
                //OSQPostFront(CommQ,"Toggle LED5");//插队
                bKeyValue = 0;
            }
            break;
