...
void USART3_SendPackage(unsigned char *data, unsigned short len)	
{
	...
	{
		while(USART_GetFlagStatus(USART3, USART_FLAG_TC) == RESET);	//等待上一个字节发送完成
		USART_SendData(USART3, *data++);							//发送一个字节
	}
}
/*********************************************************************************************************
...
void USART3_IRQHandler(void)									
{
	...
	else if(USART_GetITStatus(USART3, USART_IT_IDLE) == SET)		//如果触发了空闲中断，那么字符串接收完成
	{
		...
		Usart3.RecFlag = 1;											//标示位置1，通知主程序进行处理
...
void Usart3_Init(unsigned int bound)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB,  ENABLE);		//使能PA外设时钟

	GPIO_PinAFConfig(GPIOB,GPIO_PinSource10, GPIO_AF_USART3);	//PA9复用USART3功能
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource11, GPIO_AF_USART3);	//PA10复用USART3功能
