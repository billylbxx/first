...
u8 spi_r_w_data(u8 ch)
{
	u8 DAT;
	//LCD_CS=0;
	while(SPI_I2S_GetFlagStatus(SPI1,SPI_I2S_FLAG_TXE)==0);//确保发生前Buffer为空，也就是说上一次已经发生完成
...
{
	LCD_WR_REG(0x2a);
	LCD_WR_DATA8(x0>>8);//送高字节
	LCD_WR_DATA8(x0);//送低字节
	LCD_WR_DATA8(x1>>8);
	LCD_WR_DATA8(x1);
	LCD_WR_REG(0x2b);
	LCD_WR_DATA8(y0>>8);
...
void show_pic(u8 x0, u8 y0,u8 wide, u8 high,const unsigned char* pic)
{
	...
	{
		point_color(*(pic+1)<<8|*pic);//两个元素16位
		pic+= 2;
	}
}
